...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func favoriteTweet(tweetId:Int, success: @escaping () -> (), failture: @escaping (Error) -> ()) {
        let url = "https://api.twitter.com/1.1/favorites/create.json"
        TwitterAPICaller.client?.post(url, parameters: ["id":tweetId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in success()}, failure: { (task: URLSessionDataTask?, error: Error) in failture(error)})
    }
    
    func unfavoriteTweet(tweetId:Int, success: @escaping () -> (), failture: @escaping (Error) -> ()) {
